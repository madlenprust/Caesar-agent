...
class Orchestrator:
    ...
    _ACTION_INTENT_RE = re.compile(
        ...
        r"посмотрю|открою|скачаю|запрошу|опрошу|попробую|приступаю|"
        r"восстановлю|верн[уё]|починю|исправлю|обновлю|перезапущу|перечитаю|"
        r"добавлю|удалю|создам|перенесу|перепроверю)\b",
        re.IGNORECASE,
    )
    ...
    async def _run_with_llm(self, task: Task) -> str:
        ...
        for step in range(1, max_steps + 1):
            ...
            # или вопрос-разрешение ("восстановлю?") без tool-call и ничего ещё не
            # сделала — не сдаёмся как "финальный ответ", а пинаем саму себя действовать.
            if (not resp.tool_calls and (resp.content or "").strip()
                    and not has_used_tools and step <= 2
                    and self._ends_with_permission_question(resp.content)):
