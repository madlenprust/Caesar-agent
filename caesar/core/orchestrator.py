...
class Orchestrator:
    ...
    # Будущее время (сделаю) И инфинитив (сделать?) — агент может спросить любой формой.
    _ACTION_INTENT_RE = re.compile(
        r"\b(найд[уё]|найти|сделаю|сделать|выполню|выполнить|подключусь|подключиться|"
        r"проверю|проверить|запущу|запустить|установлю|установить|настрою|настроить|"
        r"посмотрю|посмотреть|открою|открыть|скачаю|скачать|запрошу|запросить|"
        r"опрошу|опросить|попробую|попробовать|приступаю|приступить|"
        r"восстановлю|восстановить|верн[уё]|вернуть|починю|починить|исправлю|исправить|"
        r"обновлю|обновить|перезапущу|перезапустить|перечитаю|перечитать|"
        r"добавлю|добавить|удалю|удалить|создам|создать|перенесу|перенести|"
        r"перепроверю|перепроверить)\b",
        re.IGNORECASE,
    )
    ...
    def _ends_with_permission_question(self, text: str) -> bool:
        """True если текст заканчивается вопросом-разрешением ('сделать?',
        'восстановлю?', 'продолжить?') — агент спросил разрешения и стопорится
        вместо действия. Self-poke пнёт его делать, а не спрашивать.
        """
        if not text:
            return False
        tail = text.strip().split("\n")[-1].strip()
        # последняя строка содержит '?' И глагол-обещание (найду/сделаю/восстановлю/…)
        if "?" not in tail:
            return False
        return bool(self._ACTION_INTENT_RE.search(tail[-120:]))
    ...
    async def _run_with_llm(self, task: Task) -> str:
        ...
        for step in range(1, max_steps + 1):
            ...
            # Self-poke: LLM вернула ТЕКСТ-обещание/вопрос-разрешение ("найду все машины",
            # "Сейчас восстановлю?") без tool-call и ничего ещё не сделала — не сдаёмся
            # как "финальный ответ", а пинаем саму себя действовать.
            content = (resp.content or "").strip()
            if (not resp.tool_calls and content
                    and not has_used_tools and step <= 2
                    and (self._ends_with_permission_question(content)
                         or (len(content) < 200 and self._looks_like_action_intent(content)))):
